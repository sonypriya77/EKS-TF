...
# Get ALL subnets first to check their availability zones
...
# Get subnet details to filter by supported AZs
data "aws_subnet" "all" {
...
# Create node group - node groups CAN use any subnet including us-east-1e
resource "aws_eks_node_group" "example" {
  cluster_name    = aws_eks_cluster.example.name
  node_group_name = "Node-cloud"
  node_role_arn   = aws_iam_role.example1.arn
  subnet_ids      = data.aws_subnets.all.ids  # Nodes can be in any subnet
